...
async def test_camera_capture_ssh_path(mock_bridge, mock_ssh):
    ...
    cmd = (
        'python3 -c "import cv2, base64; cap=cv2.VideoCapture(0); ret,f=cap.read(); '
        "cap.release(); _,b=cv2.imencode('.jpg',f); "
        'print(base64.b64encode(b.tobytes()).decode())"'
    )
