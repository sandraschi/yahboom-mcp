...
def probe_camera():
    ...
    out, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && ros2 node list | grep -v \"/\"'")
    ...
    ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && timeout 10 ros2 launch usb_cam camera.launch.py' &")
    
    time.sleep(12)
    
    print("[*] Performing Topic Discovery...")
    topics, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && ros2 topic list'")
    print("-" * 40)
