...
def diag_camera():
    ...
    out, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && ros2 node info /usb_cam'")
    print("-" * 40)
    print(out)
    print("-" * 40)
    ...
    out, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && ros2 topic list -t'")
    print("-" * 40)
    print(out)
    print("-" * 40)
    ...
    out, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && timeout 5 ros2 topic echo /image_raw/compressed --count 1'")
    if out.strip():
    ...
    out, _, _ = ssh.execute("docker exec yahboom_ros2 bash -c 'source /opt/ros/humble/setup.bash && ros2 topic list | grep camera'")
    print(out)
