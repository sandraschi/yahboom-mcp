...
def audit_camera_system_v2():
    ...
    p_out, _, _ = ssh.execute("sudo bash -c 'source /root/yahboomcar_ws/install/setup.bash && ros2 pkg list | grep -Ei \"camera|v4l|usb\"'")
    ...
    t_out, _, _ = ssh.execute("sudo bash -c 'source /root/yahboomcar_ws/install/setup.bash && ros2 topic list | grep -i image'")
