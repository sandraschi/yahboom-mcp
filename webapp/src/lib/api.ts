...
export const api = {
	...
	postMove: (linear: number, angular: number, linearY?: number) =>
		request<{
			status: string;
			command: { linear: number; angular: number; linear_y?: number };
		}>(
	...
	getOllamaStatus: () =>
		request<OllamaStatus>("/api/v1/settings/ollama/status"),
	getOllamaModels: () =>
		request<OllamaModelsResponse>("/api/v1/settings/ollama/models"),
	getLmStudioStatus: () =>
		request<OllamaStatus>("/api/v1/settings/lmstudio/status"),
	getLmStudioModels: () =>
		request<OllamaModelsResponse>("/api/v1/settings/lmstudio/models"),
	getLlmSettings: () => request<LLMSettings>("/api/v1/settings/llm"),
	putLlmSettings: (model: string, provider?: string) =>
	...
	postDisplayControl: (
		action: "write" | "scroll" | "clear",
		text?: string,
		line?: number,
	) => {
		if (action === "clear")
			return request<HardwareOpResponse>("/api/v1/display/clear", {
				method: "POST",
			});
		if (action === "scroll")
	...
	postMissionRun: (missionId: string) =>
		request<{ success: boolean; mission: string }>(
			`/api/v1/missions/run/${missionId}`,
			{
				method: "POST",
			},
		),
	...
	postRestartRos: () =>
		request<{ success: boolean; message: string }>(
			"/api/v1/diagnostics/ros/restart",
			{
				method: "POST",
			},
		),
	...
	gpioStatus: () =>
		request<{ success: boolean; pins: Record<string, boolean> }>(
			"/api/v1/gpio",
		),
	getDiagLogs: (lines = 80) =>
		request<{ success: boolean; logs: string }>(
			`/api/v1/diagnostics/logs?lines=${lines}`,
		),
	...
	tapoSpeak: (text: string) =>
		request<{ success: boolean }>("/api/v1/tapo/audio/speak", {
			method: "POST",
			body: JSON.stringify({ text }),
		}),
	tapoListen: (durationSec = 5) =>
		request<{ success: boolean; text: string }>("/api/v1/tapo/audio/listen", {
			method: "POST",
