...
const SlamMap: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full py-4 px-4 sm:px-6 max-w-6xl mx-auto">
      ...
      {noMap && (
        <div className="mb-4 rounded-2xl border border-amber-500/30 bg-amber-950/20 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-amber-300 font-semibold text-sm">No map data yet</p>
              <p className="text-slate-400 text-xs mt-1">
                Run the "explore_and_map" mission to start mapping, or verify slam_toolbox is
                publishing /map.
              </p>
            </div>
            <button
      ...
      {/* Map canvas */}
      <div className="relative flex-1 bg-[#0f0f12]/80 border border-white/10 rounded-2xl p-4 overflow-auto min-h-0">
        <div
          style={{
        ...
        <div className="flex items-center gap-4 text-slate-400 text-sm">
          {data && (
            <>
              ...
              <span>
                Scan pts: <span className="text-white font-mono">{data.scan_points.length}</span>
              </span>
            </>
          )}
        </div>
