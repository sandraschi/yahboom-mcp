    ...
    <div className="flex flex-col gap-6 p-4 lg:p-8 animate-in fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Connection health, telemetry, and stack diagnostics.
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-start gap-4">
          {allGood ? (
            ...
            <Radio className="w-9 h-9 text-amber-400 shrink-0 mt-0.5" />
          ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          ...
          sub={`${telemetry?.voltage ?? "—"}V`}
        />
        <StatCard
          icon={<Compass className="w-5 h-5 text-indigo-400" />}
          label="Heading"
          value={telemetry?.imu?.heading != null ? `${telemetry.imu.heading}°` : "—"}
          sub={`yaw ${telemetry?.imu?.yaw ?? "—"}°`}
        />
        <StatCard
          icon={<Navigation className="w-5 h-5 text-cyan-400" />}
          label="Velocity"
          value={`${(telemetry?.velocity?.linear ?? 0).toFixed(2)} m/s`}
          sub={`${(telemetry?.velocity?.angular ?? 0).toFixed(2)} rad/s`}
        />
        <StatCard
          icon={<Gauge className="w-5 h-5 text-amber-400" />}
          label="Sonar"
          value={telemetry?.sonar_m != null ? `${telemetry.sonar_m}m` : "—"}
          sub={
            telemetry?.scan?.nearest_m != null ? `lidar ${telemetry.scan.nearest_m}m` : "no lidar"
          }
        />
      </div>
      ...
      {health?.stack && (
        <motion.section
          layout
          className="rounded-3xl border border-white/10 bg-[#0f0f12]/80 p-5 lg:p-6 backdrop-blur-xl"
        >
          ...
          <div className="flex items-center gap-3 text-sm text-slate-400">
            <span>
              ...
              <span className="text-white font-mono">
                {(health.system.uptime / 60).toFixed(0)} min
              </span>
            </span>
            <span className="text-slate-600">|</span>
            <span>
              Version: <span className="text-white font-mono">{health.system.version}</span>
            </span>
          </div>
        </div>
      )}
