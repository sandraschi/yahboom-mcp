...
export default function Dashboard() {
	...
	useEffect(() => {
		...
		recognition.onresult = (event: any) => {
			...
			if (
				!wakeWordActive &&
				!wakeWordSeen &&
				/\bb(oo|u)mi\s*b(oo|u)mi\b/i.test(current.toLowerCase())
			) {
				setWakeWordSeen(true);
				setWakeWordActive(true);
				setLlmResponse("Bumi Bumi? I'm listening!");
				// Re-arm after 30s of inactivity
				if (wakeCommandTimeout.current)
					clearTimeout(wakeCommandTimeout.current);
			...
			if (wakeWordActive && event.results[event.results.length - 1].isFinal) {
				...
				const clean = final
					.replace(/\bb(?:oo|u)mi\s*b(?:oo|u)mi\b/gi, "")
					.trim();
	...
	const partialRobot =
		!gatewayDown && !!health && !connected && rc?.ssh === "connected";
	const noRobotPath =
		!gatewayDown && !!health && !connected && rc?.ssh !== "connected";
	const robotIp = rc?.ip ?? "—";
			...
			>
				<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
					<div className="space-y-3 min-w-0">
						...
						{rc?.hint && (
							<p className="text-[10px] text-amber-200/85 max-w-3xl leading-relaxed">
								{rc.hint}
							</p>
						)}
						...
						{health?.stack && (
							<div className="max-w-4xl">
								<h3 className="text-sm font-bold text-slate-400 uppercase tracking-wide mb-2">
									Stack health — see{" "}
									<Link to="/status" className="text-indigo-400 underline">
										Status page
									</Link>{" "}
									for details
								</h3>
								<div className="flex flex-wrap gap-2">
									{health.stack.layers?.map((l: any) => (
										<span
											key={l.id}
											className={`px-2.5 py-1 rounded-lg text-xs font-medium border ${l.ok ? "border-emerald-500/30 bg-emerald-950/40 text-emerald-400" : "border-red-500/30 bg-red-950/40 text-red-400"}`}
										>
											{l.ok ? "✓" : "✗"} {l.title}
										</span>
									))}
						...
						{gatewayDown && (
							<div className="space-y-3">
								<p className="text-sm text-red-200/90 leading-relaxed max-w-3xl">
									{error}
								</p>
								<div className="flex flex-wrap gap-2">
									<a
										...
										target="_blank"
										rel="noreferrer"
										className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-500/20 border border-indigo-500/30 text-xs font-bold text-indigo-300 hover:bg-indigo-500/30 transition-colors"
									>
										<Monitor className="w-3.5 h-3.5" /> Open API docs
									</a>
									<a
										href="http://localhost:10892/api/v1/health"
										target="_blank"
										rel="noreferrer"
										className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs font-bold text-slate-300 hover:bg-white/10 transition-colors"
									>
										<Activity className="w-3.5 h-3.5" /> Health check
									</a>
									<span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-[10px] font-bold text-amber-300/80">
										Run{" "}
										<code className="px-1 py-0.5 rounded bg-amber-500/20 text-amber-200 text-[9px]">
											start.ps1
										</code>{" "}
										in repo root
									</span>
						...
						{waitingHealth && (
							<p className="text-sm text-slate-400 leading-relaxed max-w-3xl">
								Contacting this PC&apos;s yahboom-mcp gateway and reading robot
								health…
							</p>
						)}
						...
						{allGood && (
							<p className="text-sm text-emerald-100/80 leading-relaxed max-w-3xl">
								ROS bridge is live — driving, telemetry, and stream use the
								Raspbot at{" "}
						...
						{partialRobot && (
							<div className="text-sm text-amber-100/85 leading-relaxed max-w-3xl space-y-2">
								<p>
									SSH reaches the Pi, but the WebSocket ROS bridge is not
									connected (rosbridge not running, wrong{" "}
									<span className="font-mono">YAHBOOM_BRIDGE_PORT</span>, or ROS
									still starting).
								</p>
						...
						{noRobotPath && (
							<div className="text-sm text-red-100/85 leading-relaxed max-w-3xl space-y-3">
								<p className="rounded-xl border border-red-500/25 bg-red-950/50 p-3 text-red-50/95 text-sm">
									...
									Turn the Raspbot on and wait for it to boot. Join the Raspbot
									Wi‑Fi access point on this PC,{" "}
									<span className="font-semibold">or</span> connect an Ethernet
									cable between Goliath and the Pi. Until that path exists,
									Docker, systemd, and Yahboom bringup on the Pi are not in play
									for this dashboard — there is nothing here to &quot;wake&quot;
									the robot remotely. After power + network are good, use{" "}
									<span className="font-mono text-red-100/90">Reconnect</span>{" "}
									(header) or Diagnostics{" "}
									<span className="font-mono text-red-100/90">Hard Reset</span>{" "}
									if ROS still does not attach.
								</p>
								<p>
									This PC cannot reach the configured robot address — both{" "}
									<span className="font-mono">ROS</span> (rosbridge) and{" "}
									<span className="font-mono">SSH</span> are down. The gateway
									on Goliath is running; the Pi is not participating.
								</p>
								<ul className="list-disc list-inside text-xs text-red-200/80 space-y-1.5">
									<li>
										<span className="font-semibold text-red-100/90">
											Robot off or booting
										</span>{" "}
									...
									<li>
										...
										— join Wi‑Fi to the{" "}
										<span className="font-mono">Raspbot</span> access point{" "}
										<span className="inline-flex items-center gap-1">
											<Radio className="w-3 h-3 inline" />
										</span>{" "}
										<span className="font-semibold text-red-100/90">and</span>{" "}
										no Ethernet cable is plugged between Goliath and the
										Raspberry Pi.
									</li>
									<li>
										Wrong <span className="font-mono">YAHBOOM_IP</span> in{" "}
										<span className="font-mono">webapp/start.ps1</span> (or env)
										— must match the Pi on the link you use (AP vs home LAN vs
										Ethernet).
									</li>
								</ul>
							</div>
				...
				<div className="xl:col-span-2 space-y-6">
						...
						<div className="absolute bottom-8 left-8 flex items-center gap-4 bg-black/40 backdrop-blur-lg p-4 rounded-3xl border border-white/10">
							<div className="relative w-12 h-12 flex items-center justify-center">
								...
								<Compass
									...
									style={{
										transform: `rotate(${telemetry?.imu?.heading || 0}deg)`,
									}}
								/>
							</div>
							<div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
						...
						<TelemetryCard
							...
							label="Obstacles"
							value={
								telemetry?.scan?.nearest_m
									? `${telemetry.scan.nearest_m}m`
									: "Clear"
							}
							subValue="LIDAR Active"
							trend={
								telemetry?.scan?.nearest_m && telemetry.scan.nearest_m < 0.5
									? "danger"
									: "stable"
							}
						/>
					...
					<div className="space-y-4">
							...
							<div className="flex items-center justify-between">
								...
								<button
									onClick={() => {
										const nv = !headlight;
										setHeadlight(nv);
										api.gpioSet("headlight", nv);
									}}
									className={`w-12 h-6 rounded-full transition-all ${headlight ? "bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.5)]" : "bg-slate-700"}`}
									title={headlight ? "Turn off headlight" : "Turn on headlight"}
								>
									<div
										className={`w-5 h-5 rounded-full bg-white shadow-md transition-transform ${headlight ? "translate-x-6" : "translate-x-0.5"}`}
									/>
								</button>
							</div>
						</div>
						...
						<div className="bg-[#0f0f12]/80 border border-white/5 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
							...
							<div className="flex items-center gap-2">
								...
								<button
									onClick={tapoSend}
									title="Speak through Tapo"
									className="px-3 py-1.5 bg-cyan-500/20 border border-cyan-500/30 rounded-lg text-[10px] font-bold text-cyan-400 hover:bg-cyan-500/40 transition-all"
								>
									Speak
								</button>
								<button
									onClick={tapoListen}
									disabled={tapoListening}
									title="Listen via Tapo microphone"
									className="px-3 py-1.5 bg-purple-500/20 border border-purple-500/30 rounded-lg text-[10px] font-bold text-purple-400 hover:bg-purple-500/40 transition-all disabled:opacity-50"
								>
							...
							<div className="space-y-4">
								...
								{isListening && wakeWordActive && (
									<div className="text-center py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl text-xs text-emerald-300">
										Awake — speak a command, or say <strong>"stop"</strong> to
										dismiss
									</div>
								)}
