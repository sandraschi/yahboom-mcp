...
export default function Dashboard() {
  ...
  // REST telemetry + health with exponential backoff on failure
  useEffect(() => {
    let alive = true;
    let backoffIndex = 0;
    const backoffIntervals = [1, 2, 4, 8, 16];
    ...
    const poll = async () => {
      ...
      if (alive) {
        const delay = (backoffIntervals[backoffIndex] ?? 16) * 1000;
        if (error) backoffIndex = Math.min(backoffIndex + 1, backoffIntervals.length - 1);
  ...
  return (
    <div
      ...
      className="flex flex-col gap-6 p-4 lg:p-8 animate-in fade-in duration-700"
    >
            ...
            {gatewayDown && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    <RefreshCw className="w-3.5 h-3.5" /> Restart Backend
                  </button>
                  <a
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div
          ...
          className="bg-[#0f0f12]/80 border border-white/5 rounded-3xl p-5 backdrop-blur-xl shadow-xl"
        >
          <p className="text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1">
            Server
          </p>
          <p className="text-sm font-black text-white tracking-tight">yahboom-mcp</p>
          <p className="text-[10px] text-slate-600 font-medium mt-1">
            v{health?.system?.version ?? "—"}
          </p>
        </div>
        <div
        ...
        >
          ...
          <p className="text-sm font-black text-white tracking-tight">{rc?.ip ?? "—"}</p>
          <p className="text-[10px] text-slate-600 font-medium mt-1">ROS {rc?.ros ?? "—"}</p>
        </div>
        <div
        ...
        >
          ...
          <p className="text-sm font-black text-white tracking-tight">
            {health?.stack?.layers?.length ?? "—"}
          </p>
          <p className="text-[10px] text-slate-600 font-medium mt-1">Stack layers</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#0f0f12]/80 border border-white/5 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              ...
              {tapoTranscript && (
                <p className="text-[10px] text-slate-400 mt-2">"{tapoTranscript}"</p>
              )}
            </div>
            ...
            <div className="bg-[#0f0f12]/80 border border-white/5 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  title={wasdActive ? "Disable keyboard drive" : "Enable keyboard drive (WASD)"}
