import { Battery, Compass, MapPin, Trash2, Wifi, WifiOff } from "lucide-react";
...
const MapPage: React.FC = () => {
  ...
  const heading =
    data?.imu && typeof (data.imu as { heading?: number }).heading === "number"
  ...
  useEffect(() => {
    ...
    const dist = last != null ? Math.hypot(x - last.x, y - last.y) : TRAIL_MIN_DISTANCE_M + 1;
  ...
  const standardPrompts = [
    {
      label: "Patrol the apartment",
      prompt:
        "Patrol the apartment: do a full circuit of the main rooms, avoid obstacles, and return to start. Use the agentic workflow or step-by-step motion.",
    },
    {
      label: "Go to recharge",
      prompt:
        "Go to recharge: drive to the charging station and stop. (Contactless recharger will be equipped later; for now position the robot at the dock.)",
    },
  ];
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Robot position and movement trail (odometry)</p>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 bg-[#0f0f12]/80 border border-white/10 rounded-2xl p-4 overflow-hidden">
          ...
          >
            <defs>
              <pattern
                id="grid"
                width={1 / METRES_PER_PX}
                height={1 / METRES_PER_PX}
                patternUnits="userSpaceOnUse"
              >
                <path
                  d={`M ${1 / METRES_PER_PX} 0 L 0 0 0 ${1 / METRES_PER_PX}`}
                  fill="none"
                  stroke="#1e1e2e"
                  strokeWidth="0.5"
                />
