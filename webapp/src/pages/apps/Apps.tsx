...
import { Box, Globe, ExternalLink, ShieldCheck } from 'lucide-react';

const Apps: React.FC = () => {
    const apps = [
        { name: 'Advanced Memory', desc: 'Neural knowledge graph and long-term storage.', status: 'Active' },
        ...
        <div className="space-y-8 py-4 px-4 sm:px-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-400 text-sm">Real-time Fleet Discovery and navigation for MCP services.</p>
                </div>
            </div>
                    ...
                    >
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                <Globe className={`${app.status === 'Primary' ? 'text-white' : 'text-slate-500'} w-5 h-5`} />
                                {app.status === 'Active' && <ShieldCheck className="text-green-400 w-4 h-4" />}
                                {app.status === 'Primary' && <span className="text-[10px] font-bold text-white/50 uppercase tracking-widest">Active System</span>}
                            </div>
                            <h3 className={`text-lg font-bold ${app.status === 'Primary' ? 'text-white' : 'text-slate-200'}`}>{app.name}</h3>
                            <p className={`text-xs font-medium leading-relaxed mt-2 ${app.status === 'Primary' ? 'text-indigo-100/70' : 'text-slate-500'}`}>{app.desc}</p>
                        </div>
                        {app.status !== 'Primary' && (
