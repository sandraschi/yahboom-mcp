...
const Code = ({ children }: { children: string }) => {
  ...
  return (
    <div className="relative group/code mt-2">
      ...
      <button
        onClick={copy}
        className="absolute top-2 right-2 opacity-0 group-hover/code:opacity-100 transition-opacity p-1.5 rounded-lg bg-white/10 hover:bg-white/20"
      >
        {copied ? (
...
const Accordion = ({
  ...
  return (
    <div
      className={`border rounded-2xl overflow-hidden transition-all ${open ? `border-${color}-500/40 bg-${color}-500/5` : "border-white/5 bg-[#0f0f12]/60"}`}
    >
      <button
        onClick={() => setOpen((o) => !o)}
...
const ApiRow = ({ method, path, desc }: { method: string; path: string; desc: string }) => {
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
      <span
        className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest flex-shrink-0 mt-0.5 ${colors[method] ?? "text-slate-400 bg-white/5"}`}
      >
        {method}
      </span>
  ...
  <div className="space-y-6">
    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5">
      ...
      <p className="text-[13px] text-slate-400 leading-relaxed mb-2">
        Yahboom builds ROS-based educational and research robots, expansion boards, and kits.
        Product lines include the <strong className="text-slate-300">Raspbot</strong> (Raspberry Pi
        + ROS 2), <strong className="text-slate-300">G1</strong> (humanoid/other), and the{" "}
        <strong className="text-slate-300">ROSMASTER</strong> expansion board used on many of their
        wheeled bots. This dashboard targets the{" "}
        <strong className="text-amber-200">Raspbot v2</strong>.
      </p>
      <a
        href="https://www.yahboom.net"
        target="_blank"
    ...
    <div className="space-y-3">
      ...
      <ul className="text-[13px] text-slate-400 space-y-2 list-disc list-inside">
        <li>
          <strong className="text-slate-300">SBC:</strong> Raspberry Pi 5 (recommended) or Pi 4
        </li>
        <li>
          <strong className="text-slate-300">OS image:</strong> Yahboom Raspbot image with ROS 2
          Humble (and usually rosbridge_suite) pre-installed
        </li>
        <li>
          <strong className="text-slate-300">Chassis:</strong> Mecanum wheels; motion via{" "}
          <code className="text-indigo-400">/cmd_vel</code> (Twist)
        </li>
        <li>
          <strong className="text-slate-300">Low-level control:</strong> ROSMASTER expansion board
          (STM32F103) — motors, encoders, 9-axis IMU, UART to Pi
        </li>
        <li>
          <strong className="text-slate-300">Sensors (typical):</strong> IMU (heading/pitch/roll),
          wheel encoders (odometry), optional USB/CSI camera, optional LIDAR (e.g. MS200) on{" "}
          <code className="text-indigo-400">/scan</code>
        </li>
        <li>
          <strong className="text-slate-300">Network:</strong> WiFi (AP or STA); default robot IP
          often 192.168.0.250 (Ethernet) or 192.168.1.x (WiFi)
        </li>
        <li>
          <strong className="text-slate-300">ROSBridge:</strong> WebSocket on port 9090; this
          dashboard connects from your PC to that port
        </li>
      </ul>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="border border-white/10 rounded-2xl p-4 bg-white/5">
        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
          ROSMASTER board (STM32)
        </h4>
        <p className="text-[12px] text-slate-400 leading-relaxed">
          STM32F103RCT6 (ARM Cortex-M3, 72 MHz). Runs firmware/RTOS only — no Linux. Handles motor
          PID, encoders, IMU, servos, UART to Pi. Camera and LIDAR run on the Pi, not on the STM32.
        </p>
      </div>
      <div className="border border-white/10 rounded-2xl p-4 bg-white/5">
        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
        ...
        <p className="text-[12px] text-slate-400 leading-relaxed">
          <strong className="text-slate-300">Full Pi (Pi 5):</strong> ROS 2 + rosbridge on robot,
          camera, optional LIDAR. <strong className="text-slate-300">Minimal Pi:</strong>{" "}
          camera/stream only. <strong className="text-slate-300">Pi-less:</strong> chassis + ESP32;
          PC does all compute (no camera/LIDAR on bot).
        </p>
      </div>
    </div>
    ...
    <div className="space-y-2">
      ...
      <p className="text-[13px] text-slate-400">
        Yahboom MS200 TOF LIDAR — 360°, ~0.03–12 m, USB/serial. ~$139 USD. ROS 2 driver publishes{" "}
        <code className="text-indigo-400">/scan</code>; dashboard Lidar Map and telemetry use it.
        See <strong className="text-slate-300">Lidar addon</strong> page and{" "}
        <a
          href="https://www.yahboom.net/study/MS200"
          target="_blank"
          rel="noopener noreferrer"
          className="text-indigo-400 hover:underline"
        >
          yahboom.net/study/MS200
        </a>
        .
      </p>
    </div>
    ...
    <div className="text-[12px] text-slate-500">
      More: <strong className="text-slate-400">docs/HARDWARE_AND_ROS2.md</strong> (Pi tiers, ROS 2
      interaction, LIDAR), <strong className="text-slate-400">docs/CONNECTIVITY.md</strong> (WiFi,
      robot IP, ROSBridge at boot).
    </div>
  </div>
);
  ...
  <div className="space-y-5">
    <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-2xl p-5">
      ...
      <ul className="text-[13px] text-slate-400 space-y-1 list-disc list-inside">
        ...
        <li>
          ROSBridge WebSocket server on the robot (pre-installed; starts at boot if you ran the
          one-time script)
        </li>
    ...
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
        Step 1 — ROSBridge on the robot
      </p>
      <p className="text-[12px] text-slate-500">
        Stock Raspbot image already has rosbridge. If it is not set to start at boot, run once on
        the Pi:
      </p>
      <Code>ros2 launch rosbridge_server rosbridge_websocket_launch.xml</Code>
      <p className="text-[12px] text-slate-500">
        To start automatically at boot, use the one-time script in{" "}
        <strong className="text-slate-400">docs/ROSBRIDGE_AT_BOOT.md</strong>.
      </p>
    </div>
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
        Step 2 — Configure robot IP (if needed)
      </p>
      <Code>$env:YAHBOOM_IP = "192.168.1.100" $env:YAHBOOM_BRIDGE_PORT = "9090"</Code>
    </div>
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
    ...
    <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-5">
      ...
      <p className="text-[13px] text-slate-400 mb-2">
        Open the health endpoint — <code className="text-green-400">connected: true</code> means the
        bot is linked.
      </p>
    ...
    <div className="grid grid-cols-2 gap-4">
      {[
        ...
        { label: "Dashboard", url: "http://localhost:10893/", icon: Activity },
      ].map((l) => (
        <a
          key={l.label}
          href={l.url}
  ...
  <div className="space-y-6">
    <div className="bg-[#0f0f12]/60 border border-white/5 rounded-2xl p-5">
      <p className="text-sm text-slate-400 leading-relaxed">
        All MCP tools are available via AI clients pointed at{" "}
        <code className="text-indigo-400">http://localhost:10892/sse</code>. The main tool is{" "}
        <span className="text-white font-bold">yahboom()</span> — a portmanteau with an{" "}
        <code className="text-indigo-400">action</code> parameter that routes to sub-operations.
      </p>
    </div>
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Motion Control</p>
      <ToolRow
        call='yahboom(action="move", linear=0.2, angular=0.0)'
        desc="Move forward at 0.2 m/s"
        example='yahboom(action="move", linear=0.3, angular=0.0)   # forward
yahboom(action="move", linear=0.0, angular=0.5)   # turn left
yahboom(action="move", linear=0.0, angular=0.0)   # STOP'
      />
      <ToolRow
        call='yahboom(action="move_to", x=1.5, y=0.0)'
        desc="Autonomous waypoint navigation (requires odometry)"
      />
    </div>
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
      ...
      <ToolRow
        call='yahboom(action="read_imu")'
        desc="Returns heading (°), pitch, roll from 9-axis IMU"
      />
      <ToolRow
        call='yahboom(action="health")'
        desc="Returns bridge connection state and battery percentage"
      />
    </div>
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
        Help System (drill-down)
      </p>
      <ToolRow
        call="yahboom_help()"
        desc="List all help categories"
        example='yahboom_help()                              # list categories
...
yahboom_help(category="motion", topic="mecanum")  # full detail'
    ...
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
        MCP Client config (mcp_config.json)
      </p>
      <Code>{`{
  "mcpServers": {
    "yahboom": {
    ...
    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 text-[13px] text-slate-400">
      ...
      linear ±1.0 m/s · angular ±2.0 rad/s. Values outside range are clamped by firmware — no hard
      errors.
    </div>
  </div>
);
  ...
  <div className="space-y-6">
    ...
    <div className="space-y-3">
      ...
      <ApiRow
        ...
        desc="Battery %, IMU heading, wheel velocity (bridge must be connected)"
      />
      <ApiRow
        method="POST"
        path="/api/v1/control/move"
        desc="Direct motion: ?linear=0.2&angular=0.0"
      />
      <ApiRow method="GET" path="/stream" desc="MJPEG video stream — embed as <img src>" />
      <ApiRow
        method="GET"
        path="/sse"
        desc="MCP over Server-Sent Events — for AI client connections"
      />
...
const TabConnect = () => (
  <div className="space-y-5">
    <Accordion title="What's needed on the robot side?" icon={Cpu} color="indigo">
      <p>
        Yahboom Raspbot v2 (Raspberry Pi 5 or 4) with stock image: ROS 2 Humble and rosbridge_suite
        are pre-installed. ROSBridge must be running (at boot or manually):
      </p>
      <Code>ros2 launch rosbridge_server rosbridge_websocket_launch.xml</Code>
      <p>
        Default port: <strong className="text-slate-300">9090</strong>. Robot and workstation must
    ...
    <Accordion title="How do I set the robot IP?" icon={Wifi} color="indigo">
      ...
      <p>
        Or use the <strong className="text-slate-300">Onboarding</strong> page in the webapp to save
        these settings.
      </p>
    </Accordion>
    <Accordion title="What do the server run modes mean?" icon={Terminal} color="indigo">
      <ul className="space-y-2">
        <li>
          <strong className="text-slate-300">stdio</strong> — MCP only (default for AI client use
          via mcp_config.json)
        </li>
        <li>
          <strong className="text-slate-300">http</strong> — FastAPI + SSE only (REST + MCP, no
          stdio)
        </li>
        <li>
          <strong className="text-slate-300">dual</strong> — both stdio and HTTP. Used by start.ps1
        </li>
      </ul>
    </Accordion>
    <Accordion title="How do AI clients connect?" icon={Radio} color="indigo">
      <p>
        Point your <code className="text-indigo-400">mcp_config.json</code> to the SSE endpoint:
      </p>
  ...
  <div className="space-y-4">
    ...
    <Accordion title="How does obstacle avoidance work?" icon={Activity} color="indigo">
      ...
      <ol className="list-decimal list-inside text-sm text-slate-400 mt-2 space-y-1">
        ...
        <li>Turns for 2 seconds to clear the obstacle</li>
        <li>Resumes the search pattern</li>
        <li>Publishes was_blocked=true in mission_status</li>
      </ol>
    </Accordion>
    <Accordion title="Ollama & vision detection" icon={Cpu} color="indigo">
      <p className="text-sm text-slate-400 leading-relaxed">
        Missions use <strong>Gemma3:1b</strong> running on the Pi via Ollama (192.168.1.11:11434).
    ...
    {[
      {
        icon: AlertTriangle,
        color: "amber",
        title: '"Robot Not Connected" banner showing',
        body: "Backend is up but ROSBridge is unreachable.",
        fix: [
          "1. Confirm robot is powered on",
          "2. SSH to Pi and run rosbridge: ros2 launch rosbridge_server rosbridge_websocket_launch.xml",
          "3. Ping the robot IP from your workstation",
      ...
      },
      {
        icon: Zap,
        color: "red",
        title: '"Server Down" banner / cannot reach port 10892',
        body: "Python backend has not started or crashed.",
        fix: [
          "Run .\\start.ps1 (or start.bat)",
          "Check terminal for Python tracebacks",
          "Test: curl http://localhost:10892/api/v1/health",
          "Port conflict: netstat -ano | Select-String 10892 — kill the PID",
        ],
      },
      {
        icon: Terminal,
        color: "red",
        title: "npm Win32 error during start",
        body: "Vite cannot start because npm.cmd is not a Win32 exe — start.ps1 must invoke it via cmd /c.",
        fix: [
          'In start.ps1 the line must be: Start-Process cmd -ArgumentList "/c", "npm", "run", "dev"',
        ],
      },
      {
        icon: Activity,
        color: "indigo",
        title: "Dashboard blank / black after first load",
        body: "A TypeError inside a React component is unmounting the tree.",
        fix: [
          "Open browser DevTools (F12) → Console → find red TypeError",
          "Common cause: .toFixed() on undefined telemetry field",
          "Ensure main.tsx wraps App in <BrowserRouter>",
        ],
      },
      {
        icon: BookOpen,
        color: "indigo",
        title: "FastMCP AttributeError: no attribute .app",
        body: "FastMCP 3.0 removed the .app property.",
        fix: [
          "Use FastAPI-first pattern: create app = FastAPI(...) then mcp = FastMCP.from_fastapi(app)",
        ],
      },
    ].map((item, i) => (
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Complete reference for the Yahboom ROS 2 MCP server and dashboard.
          </p>
        </div>
      </div>
