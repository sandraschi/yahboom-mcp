import { AlertCircle, Gamepad2, Music, Navigation, Square, WifiOff } from "lucide-react";
...
const MovementPage: React.FC = () => {
  ...
  const Section: React.FC<{ title: string; icon: React.ReactNode; items: Preset[] }> = ({
    title,
    icon,
    items,
  }) => (
  ...
  return (
    <div className="flex flex-col h-full py-4 px-4 sm:px-6 max-w-5xl mx-auto">
      ...
      <div className="space-y-6">
        <Section
          title="Preset movements"
          icon={<Square className="w-4 h-4 text-indigo-400" />}
          items={PRESETS}
        />
        <Section
          title="Patrolling"
