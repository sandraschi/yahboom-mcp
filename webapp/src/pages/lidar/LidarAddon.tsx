import { AlertCircle, CheckCircle2, ExternalLink, Package, ScanLine } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const scan = (
          t as { scan?: { nearest_m?: number | null; obstacles?: Record<string, unknown> } }
        ).scan;
        const hasScan =
          scan &&
          (scan.nearest_m != null ||
            (scan.obstacles && Object.values(scan.obstacles).some((v) => v != null)));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5">
          ...
          <ul className="text-sm text-slate-300 space-y-2">
            <li>
              <strong className="text-white">Yahboom MS200</strong> TOF LIDAR — 360°, 0.03–12 m
            </li>
            <li>
              ROS 1 / ROS 2 drivers; publishes{" "}
              <code className="bg-white/10 px-1 rounded">/scan</code>
            </li>
            <li>
              <strong className="text-emerald-400">~${MS200_PRICE_USD} USD</strong> (check store for
              current price)
            </li>
          </ul>
          <a
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-slate-300">
                /scan data:{" "}
                {scanActive === true ? "Active (LIDAR or scan topic publishing)" : "No data yet"}
              </span>
            </div>
          </div>
          <p className="text-xs text-slate-500 mt-4">
            When the MS200 is installed and the ROS 2 driver is running on the Pi, /scan will
            publish and this page and the Lidar Map will show live data.
          </p>
        </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5">
          ...
          <ol className="list-decimal list-inside text-sm text-slate-300 space-y-2">
            ...
            <li>
              On the Pi: install Yahboom’s MS200 ROS 2 driver and run it so{" "}
              <code className="bg-white/10 px-1 rounded">/scan</code> is published.
            </li>
            <li>
              Start rosbridge on the robot as usual. This dashboard already subscribes to /scan; the{" "}
              <strong>Lidar Map</strong> page and telemetry will show data automatically.
            </li>
          </ol>
          <p className="text-xs text-slate-500 mt-4">
            See{" "}
            ...
            and <code className="bg-white/10 px-1 rounded">docs/HARDWARE_AND_ROS2.md</code> (section
            5).
          </p>
        </div>
      </div>
